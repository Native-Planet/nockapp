                ...
                loop {
                    if header_list.is_atom() {
                        ...
                        if let Some(key) = key_vec.to_bytes_until_nul() {
                            ...
                            if let Some(val) = val_vec.to_bytes_until_nul() {
                                header_vec.push((
                                    String::from_utf8(key_vec.as_bytes().to_vec())?,
                                    String::from_utf8(val)?,
                                ));
                                header_list = header_list.as_cell()?.tail();
                            } else {
                ...
                let body: Option<Bytes> = {
                    if maybe_body.is_cell() {
                        ...
                        let body_len = body_octs
                            ...
                            .expect("body len")
                            .data();
                        let mut body_vec: Vec<u8> = vec![0; body_len.try_into().unwrap()];
                        let body_atom = body_octs.tail().as_atom()?;
                        match body_atom.to_bytes_until_nul() {
                            Some(bytes) => {
                                let copy_len = body_vec.len().min(bytes.len());
                                body_vec[..copy_len].copy_from_slice(&bytes[..copy_len]);
                            },
                            None => body_vec = body_atom.as_bytes().to_vec(),
                        }
